<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:62:"F:\wamp\www\shop/application/member\view\order\orderdetail.htm";i:1543394642;s:55:"F:\wamp\www\shop/application/index\view\common\head.htm";i:1526908574;s:56:"F:\wamp\www\shop/application/member\view\common\left.htm";i:1543755147;s:57:"F:\wamp\www\shop/application/index\view\common\footer.htm";i:1522488208;}*/ ?>
<!doctype html>
<html>
<head>
...
<div class="avatar">
    <a href="#" class="u-pic">
    <?php if($leftUserInfo['avatar_src']): ?>
        <img src="__uploads__/<?php echo $leftUserInfo['avatar_src']; ?>" alt="">
    <?php else: ?>
...
<div class="name">
    <h2><?php echo $leftUserInfo['nick_name']; ?></h2>
...
<div class="item clearfix">
    ...
    <div class="item-main">
<?php if($registerType == 1): ?>
<b class="integrity"><em style="width: 70%;"></em></b><span>70%</span>
<?php elseif($registerType == 0): ?>
<b class="integrity"><em style="width: 60%;"></em></b><span>60%</span>
<?php else: ?>
<b class="integrity"><em style="width: 100%;"></em></b><span>100%</span>
<?php endif; ?>
    </div>
</div>
<div class="item clearfix">
...
<?php if(($registerType == 0) OR ($registerType == 2)): ?>
<a href="#" class="iconfont icon-email active"><span class="tip">邮箱已验证</span></a>
<?php else: ?>
...
<?php if(($registerType == 1) OR ($registerType == 2)): ?>
<a href="#" class="iconfont icon-mobile-phone active"><span class="tip">手机已验证</span></a>
<?php else: ?>
<a href="#" class="iconfont icon-mobile-phone"><span class="tip">手机未验证</span></a>
<?php endif; ?>
    </div>
...
<dd>
    <p><a href="<?php echo url('member/Order/orderList'); ?>" target="_self">我的订单</a></p>
    <p><a href="<?php echo url('member/Address/index'); ?>" target="_self">收货地址</a></p>
</dd>
    </dl>
    <dl>
<dt><i class="square"></i><span>会员中心</span></dt>
<dd>
    <p><a href="<?php echo url('member/User/userInfo'); ?>" target="_self">用户信息</a></p>
    </dd>
    </dl>
    </div></div>
    </div>
  ...
  <div class="user-mod user-order-detail" style="min-height: 1018px;">
            <div class="user-title">
  <h2>订单详情</h2>
  <a href="user.php?act=message_list&amp;is_order=1&amp;order_id=1" class="more">留言<em>&nbsp;&nbsp;(&nbsp;0&nbsp;)</em></a>
            </div>
            <div class="order-detail-statu">
  <div class="statu-left">
    <?php if($orders['pay_status'] == 0): ?>
      <p>未付款订单 <span style="color: deepskyblue;"></span></p>
    <?php else: ?>
      <p>已付款订单 <span style="color: deepskyblue;"></span></p>
    <?php endif; ?>
      <p>订单号：<?php echo $orders['out_trade_no']; ?>      
      </p>
  </div>
  <div class="statu-right mt10">
            ...
            <ul class="order-detail-progress progress-<?php echo $orderProgress; ?>">
  <li>下单时间<div class="tip"><?php echo date("Y-m-d H:i:s",$orders['order_time']); ?></div></li>
  <li>付款<div class="tip"><?php if($orders['pay_status'] == 1): ?>已付款<?php endif; ?></div></li>
  <li>发货<div class="tip"><?php if(($orders['post_status'] == 1) OR ($orders['post_status'] == 2)): ?>已发货<?php endif; ?></div></li>
  <li>收货<div class="tip"><?php if($orders['post_status'] == 2): ?>已收货<?php endif; ?></div></li>
  <li>完成<div class="tip"><?php if($orders['order_status'] == 1): ?>已完成<?php endif; ?></div></li>
            </ul>
  ...
  <div class="info-content">
      <div class="info-item">
          ...
          <div class="item-value"><?php echo $orders['name']; ?></div>
      </div>
      <div class="info-item">
          <div class="item-label">手机号码：</div>
          <div class="item-value"><?php echo $orders['phone']; ?></div>
      </div>
      <div class="info-item">
          <div class="item-label">收货地址：</div>
          <div class="item-value"><?php echo $orders['province']; ?>  <?php echo $orders['city']; ?>  <?php echo $orders['country']; ?>&nbsp;<?php echo $orders['address']; ?></div>
      </div>
  </div>
            </div>
  ...
  <div class="info-title"><h2>付款信息</h2></div>
    <div class="info-content">
      <div class="info-item">
          ...
          <?php if($orders['payment'] == 1): ?>
          支付宝
          <?php elseif($orders['payment'] == 2): ?>
          微信
          <?php else: ?>
          余额
          <?php endif; ?>
      ...
      <?php if($orders['pay_status'] == 0):?>
      <div class="info-item">
        <div class="item-label lh30">尚未支付：</div>
        <div class="item-value">
        <a href="<?php echo url('index/Flow/flow4',array('oid'=>$orders['id']),''); ?>" target="_blank">去支付</a>
            <!-- <form name="payment" method="post" action="user.php">
  <input type="submit" name="Submit" class="btn sc-redBg-btn btn30 ml5" value="去支付">
            </form> -->
        </div>
    </div>
  <?php else:?>
...
已支付    </div>
  ...
  <div class="info-content">
      ...
      <div class="item-value" id="shipping_name_1"><?php echo $orders['distribution']; ?></div>
      
      </div>
            <div class="info-item">
          <div class="item-label">配送状态：</div>
          <div class="item-value">
          <?php if($orders['post_status'] == 0): ?>
          未发货
          <?php elseif($orders['payment'] == 1): ?>
          已发货
          <?php else: ?>
          已收货
          <?php endif; ?>
  ...
  <thead>
      <tr>
          <th width="40%">商品</th>
          <th width="20%" class="tl">属性</th>
          <th width="10%" class="tl">货号</th>
          <th width="10%" class="tl">发货仓库</th>
  ...
  foreach($goodsRes as $k=>$v):
        $totalGoodsPrice += $v['member_price']*$v['goods_num'];
        $totalNum += $v['goods_num'];
  ?>
        <tr>
          <td>
          <a href="goods.php?id=846" class="img"><img src="__uploads__/<?php echo $v['mid_thumb']; ?>" alt=""></a>
      
<a href="goods.php?id=846" class="name"><?php echo $v['goods_name'];?></a>
<!-- <p class="fl ftx-05">商品赠送积分：0</p> -->
           
 
          </td>
          <td><?php echo $v['goods_attr_str'];?></td>
          <td><?php echo $orders['out_trade_no']; ?></td>
          <td>总部</td>
          <td><p><em>¥</em><?php echo $v['member_price']; ?></p><p>×<?php echo $v['goods_num']; ?></p></td>
          <td>
<p class="ftx-01"><em>¥</em><?php echo $v['member_price']*$v['goods_num']; ?></p>
          </td>
      </tr>
  <?php endforeach;?>
    </tbody>
...
<?php 
        $totalPrice = $orders['post_spent']+$totalGoodsPrice;
    
?>
          <div class="user-order-detail-total">
    <dl class="total-row">
      <dt class="total-label">商品件数：</dt>
      <dd class="total-value"><?php echo $totalNum;?>件</dd>
  </dl>
    <dl class="total-row">
      <dt class="total-label">商品总价：</dt>
      <dd class="total-value"><em>¥</em><?php echo $totalGoodsPrice;?></dd>
  </dl>
            
      <dl class="total-row">
      <dt class="total-label">配送费用：</dt>
      <dd class="total-value">+ <em>¥</em><?php echo $orders['post_spent']; ?></dd>
  </dl>
<dl class="total-row">
      <dt class="total-label">
  应付款：
        </dt>
      <dd class="total-value"><em>¥</em><?php echo $totalPrice;?></dd>
  </dl>
            </div>
        </div>
        <script type="text/javascript">
        ...
        function checkOrder(){
            ...
            if(pay_code == 'wxpay'){
  var url = "user.php?act=checkorder&log_id=" + log_id+ "&pay_code=" + pay_code;
  $.get(url, {}, function(data){
      //已付款
      if(data.code == 1){
          clearTimeout(timer);
          location.href = "user.php?act=order_detail&order_id="+1;
      }
  },'json');
